-- Database Migration: Enforce Cashier Shift for Cash Payments
-- Location: supabase/require_shift_for_cash.sql
...
BEGIN
  ...
  -- ── STEP 3: CASH SALES REQUIRE THE CASHIER'S OWN OPEN SHIFT ──────────────
  -- QRIS is exempt (no physical cash drawer is affected). Validate the shift 
  -- server-side; never trust p_shift_id alone. Ownership is validated against 
  -- the secure auth.uid() (JWT identity) instead of the client-supplied p_dibuat_oleh.
  IF p_payment_method = 'cash' THEN
    ...
    WHERE id = p_shift_id
      AND tenant_id = v_tenant_id       -- shift belongs to this store
      AND cashier_id = auth.uid()       -- shift belongs to THIS cashier
      AND status = 'open';              -- shift is currently open
