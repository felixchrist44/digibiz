-- Database Migration: Re-scope Invoice Uniqueness to Per-Tenant and Fix RPC Handler
-- Location: supabase/multi_tenant_invoice_fix.sql

BEGIN;

-- 1. Drop the global unique constraint on nomor_invoice (uniqueness should be per-tenant)
ALTER TABLE public.penjualan DROP CONSTRAINT IF EXISTS penjualan_nomor_invoice_key;

-- 2. Create the composite unique index to enforce uniqueness per-tenant
CREATE UNIQUE INDEX IF NOT EXISTS uq_penjualan_tenant_invoice
  ON public.penjualan (tenant_id, nomor_invoice);

-- 3. Redefine process_sale_transaction with a strict exception handler that does not swallow other unique violations
